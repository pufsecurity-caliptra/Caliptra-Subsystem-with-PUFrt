...
✅ Pre-integrated Caliptra Subsystem with PUFrt  
✅ [Official VCK190 FPGA Reference](https://github.com/pufsecurity-caliptra/caliptra-mcu-sw/tree/main/hw/fpga)  
...
A complete implementation typically requires the integration of a PUF-based UDS generation mechanism, a Noise Source for entropy, and OTP / secure storage — all of which must be validated together with the Caliptra subsystem before silicon adoption can begin.
...
The Caliptra core connects to the Fuse Controller over the internal system bus. The Fuse Controller interfaces with PUFrt through the **OTI (OpenTitan Interface)**, enabling secure access to hardware security primitives.

**PUFrt** All-in-one Physical Anchor for Caliptra. Provides the security foundation required for real silicon deployment:

- **PUF** — pre-hardened PUF for generating a unique device secret and UDS seed.
- **OTP** — qualified anti-fuse OTP for secure storage of keys and Lifecycle State (LCS).
- **Noise Source** — ESV-certified entropy source meeting NIST SP 800-90B requirements for Caliptra TRNG.
- **Access Control & Anti-Tampering** — integrated security mechanisms for enhanced physical protection.
- **Advanced Process Support** — supports advanced nodes down to 3nm, with 2nm and beyond coming soon.

PUFrt exposes two interfaces: **OTI** on the crypto side for Caliptra integration, and **APB** on the system side for host access.

[**TODO**]
- Add FPGA architecture image for the pre-integrated Caliptra Subsystem with PUFrt
...
### Technical Support & Integration Inquiry

For technical support or further information, please contact us:

📧 business@pufsecurity.com

Our team can provide dedicated support for:

- Reference Environment Issues
- Pre-integrated Caliptra + PUFrt Solution
- Standalone PUFrt Solution
- Custom Hardware Security Integration
- Further information and technical discussions

We look forward to working with you to build your own Caliptra solution.
